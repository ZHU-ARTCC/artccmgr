...
RSpec.describe ActivityHelper, type: :helper do
  describe '#activity_minimums_css' do
    before :each do
      @user = create(:user, group: create(
        :group, staff: true, min_controlling_hours: 3
      ))
    end
  ...
  describe '#report_controls' do
    it 'returns a previous month link' do
      ...
      assign(:report_start, Time.now.utc.beginning_of_month)
      assign(:report_end, Time.now.utc.end_of_month)

      expect(
        helper.report_controls[:prev]
      ).to eq activity_index_path(start_date: last_month)
    end

    it 'returns a next month link if the end time is less than now' do
      start_time = (Time.now.utc - 2.months).beginning_of_month
      end_time   = (Time.now.utc - 2.months).end_of_month
      next_month = (Time.zone.today - 1.month).beginning_of_month

      assign(:report_start, start_time)
      assign(:report_end, end_time)
      expect(
        helper.report_controls[:next]
      ).to eq activity_index_path(start_date: next_month)
    end
