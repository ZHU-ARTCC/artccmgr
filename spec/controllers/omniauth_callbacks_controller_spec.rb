...
RSpec.describe OmniauthCallbacksController, type: :controller do
  ...
  describe '#vatsim' do
    ...
    context 'with existing user' do
      before :each do
        User.destroy_all
        @user = create(:user, cid: 9_999_999)
      end
      ...
      context 'with 2FA' do
        before :each do
          User.destroy_all
          @user = create(
            :user, :two_factor_via_otp, :two_factor_via_u2f, cid: 9_999_999
          )
        end
        ...
        it 'prompts the user for the second factor' do
          ...
          expect(response).to render_template(
            'two_factor', 'sessions/two_factor'
          )
        end
  ...
  describe '#authenticate_with_two_factor' do
    ...
    context 'via OTP' do
      ...
      context 'valid attempt' do
        before do
          allow_any_instance_of(User).to(
            receive(:validate_and_consume_otp!).and_return(true)
          )
      ...
      context 'using backup code' do
        before do
          allow_any_instance_of(User).to(
            receive(:invalidate_otp_backup_code!).and_return(true)
          )
          post :authenticate_with_two_factor, params: { otp_attempt: '123456' }
        end
      ...
      context 'invalid attempt' do
        before do
          allow_any_instance_of(User).to(
            receive(:validate_and_consume_otp!).and_return(false)
          )
          post :authenticate_with_two_factor, params: { otp_attempt: '123456' }
        end
    ...
    context 'via U2F' do
      ...
      context 'valid attempt' do
        before do
          ...
          post :authenticate_with_two_factor, params: {
            user: { device_response: '1234' }
          }
        end
