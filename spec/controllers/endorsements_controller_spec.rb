...
RSpec.describe EndorsementsController, type: :controller do
	...
	describe 'POST #create' do
		context 'when not logged in' do
			it 'does not create the Endorsement' do
				user = create(:user)
				expect {
					post :create, params: { user_id: user.id, endorsement: build(:endorsement).attributes }
				}.to_not change(Endorsement, :count)
			end
		end
		...
		context 'with invalid attributes' do
			...
			it 'does not save the new certification' do
				expect{
					post :create, params: { user_id: @user.id, endorsement: build(:endorsement, :invalid).attributes }
				}.to_not change(Endorsement,:count)
			end
	...
	describe 'DELETE #destroy' do
		...
		context 'when logged in' do
			before :each do
				sign_in create(:user, group: create(:group, :perm_endorsement_read, :perm_endorsement_delete))
			end
	...
	describe 'GET #edit' do
		before :each do
			sign_in create(:user, group: create(:group, :perm_endorsement_read, :perm_endorsement_update))
			@endorsement = create(:endorsement)
		end
	...
	describe 'PUT #update' do
		...
		context 'when not logged in' do
			it 'does not update Endorsement' do
				put :update, params: { user_id: @endorsement.user, id: @endorsement, endorsement: attributes_for(:endorsement, instructor: 'Testing') }
				@endorsement.reload
		...
		context 'valid attributes' do
			...
			it 'located the requested @endorsement' do
				put :update, params: { user_id: @endorsement.user, id: @endorsement, endorsement: attributes_for(:endorsement) }
				expect(assigns(:endorsement)).to eq @endorsement
			end
		...
		context 'invalid attributes' do
			...
			it 'locates the requested @endorsement' do
				put :update, params: { user_id: @endorsement.user, id: @endorsement, endorsement: attributes_for(:endorsement, :invalid) }
				expect(assigns(:endorsement)).to eq @endorsement
			end

			it "does not change @endorsement's attributes" do
				put :update, params: { user_id: @endorsement.user,
															 id: @endorsement,
