...
RSpec.describe Events::PilotsController, type: :controller do
      ...
      it 'does not create Pilot Signup' do
        expect do
          post :create, params: {
            ...
            event_pilot: build(:event_pilot, event: event).attributes
          }
        end.to_not change(Event::Pilot, :count)
      end
    end
    ...
    context 'with valid attributes' do
      before :each do
        sign_in create(:user, group: create(
          :group, :perm_event_pilot_signup_create
        ))
      end
      ...
      it 'creates a new event pilot' do
        ...
        end.to change(Event::Pilot, :count).by 1
      end
      ...
      it 'redirects to the event #show page' do
        ...
        post :create, params: {
          event_id: event,
          event_pilot: event_pilot.attributes
        }
    ...
    context 'with invalid attributes' do
      ...
      it 'does not save the new event pilot' do
        expect do
          post :create, params: {
            event_id: event,
            event_pilot: build(:event_pilot, :invalid, event: event).attributes
          }
        end.to_not change(Event::Pilot, :count)
      end
  ...
  describe 'DELETE #destroy' do
    ...
    context 'when logged in' do
      before :each do
        sign_in create(:user, group: create(
          :group, :perm_event_pilot_signup_read, :perm_event_pilot_signup_delete
        ))
      end
      ...
      it 'redirects to the events#index path if the pilot cannot be deleted' do
        allow_any_instance_of(Event::Pilot).to(
