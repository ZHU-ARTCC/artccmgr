...
RSpec.describe ActivityController, type: :controller do
	...
	describe 'GET #index' do
		...
		context 'logged in when not staff' do
			before :each do
				sign_in create(:user, group: create(:group, :perm_user_read, staff: false))
			end
		...
		context 'logged in when staff' do
			before :each do
				sign_in create(:user, group: create(:group, :perm_user_read, staff: true))
			end
			...
			it 'accepts a custom start date' do
				...
				expect(assigns(:report_start)).to eq start_date.beginning_of_month
				expect(assigns(:report_end)).to eq start_date.end_of_month
			end
