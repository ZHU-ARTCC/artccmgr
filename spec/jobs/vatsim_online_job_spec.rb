...
RSpec.describe VatsimOnlineJob, type: :job do
  ...
  context 'before a new update is due' do
    before :each do
      ...
      @data_time    = Time.now.utc
      @update_time  = Time.now.utc + 2.minutes
    ...
    it 'should not download online user information' do
      expect(
        VatsimOnlineJob.perform_now(@data_time.to_s, @update_time.to_s)
      ).to_not have_requested(:get, %r{/.*\/vatsim-data.txt/})
    end
  ...
  context 'when no data has been downloaded or an update is due' do
    before :each do
      ActiveJob::Base.queue_adapter = :async
      @data_time    = Time.now.utc - 2.minutes
  ...
  context 'when no status URL is configured' do
    ...
    it 'should log that it is unable to get new data servers' do
      expect(Rails.logger).to(
        receive(:error).with(
          /VatsimOnlineJob: Unable to retrieve VATSIM data servers/
        )
      )

      expect(Rails.logger).to(
        receive(:error).with(/VatsimOnlineJob: No data servers available/)
      )
    ...
    it 'should retry three times' do
      expect_any_instance_of(VatsimOnlineJob).to(
        receive(:sleep).with(5).exactly(3).times
      )
      VatsimOnlineJob.perform_now
    end
  ...
  context 'when no data servers could be found' do
    before :each do
      ...
      allow(VATSIM::Data).to(
        receive(:new).and_raise(StandardError.new('rspec test'))
      )
    end
    ...
    it 'should log that it unable to download VATSIM status' do
      expect(Rails.logger).to(
        receive(:error).with(
          /VatsimOnlineJob: Unable to download VATSIM status/
        )
      )
      VatsimOnlineJob.perform_now
    end
