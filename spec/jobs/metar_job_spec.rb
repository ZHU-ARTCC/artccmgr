  ...
  context 'updating weather' do
    ...
    describe 'with an invalid ICAO identifier' do
      ...
      it 'logs the error' do
        expect(Rails.logger).to(
          receive(:error).with(/MetarJob: Unable to retrieve METAR for/)
        )
    ...
    describe 'encountering an error caused by the Metar library' do
      before do
        ...
        allow(Metar::Station).to(
          receive(:find_by_cccc).and_raise(StandardError, @msg)
        )
      end
      ...
      it 'logs the error' do
        expect(Rails.logger).to(
          receive(:error).with(
            "MetarJob: Unable to update weather for #{@icao}: #{@msg}"
          )
        )
        MetarJob.perform_now
      end
    ...
    describe '#format_altimeter' do
      ...
      it 'does not save a weather object when barometer value is missing' do
        ...
        allow(Metar::Station).to(
          receive(:find_by_cccc).and_return(OpenStruct.new(parser: parser))
        )
    ...
    describe '#format_rules' do
      ...
      it 'determines VFR with no ceiling' do
        raw = Metar::Raw::Metar.new 'XTST 311718Z 14011G15KT 6SM CLR 27/18 A3006 RMK AO2 RAB01E12 P0000 T02670183'
        parser = Metar::Parser.new(raw)
