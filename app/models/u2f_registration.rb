...
# Universal 2nd factor (U2F) device registration information, like Yubikeys
#
class U2fRegistration < ApplicationRecord
  ...
  def self.register(user, app_id, params, challenges)
    ...
    begin
      ...
      msg = 'Your U2F device did not send a valid JSON response'
      registration.errors.add(:base, msg)
    rescue U2F::Error => e
  ...
  def self.authenticate(user, app_id, json_response, challenges)
    ...
    registration = user.u2f_registrations.find_by(
