...
class EndorsementsController < ApplicationController
  ...
  def create
    ...
    if @endorsement.update_attributes(permitted_attributes(@endorsement))
      redirect_to user_path(@controller),
                  success: 'Endorsement has been created'
    else
  ...
  def destroy
    ...
    if @endorsement.destroy
      redirect_to user_path(@controller),
                  success: 'Endorsement has been revoked'
    else
      redirect_to edit_user_endorsement_path(@controller, @endorsement),
                  alert: 'Unable to delete endorsement'
    end
  end
