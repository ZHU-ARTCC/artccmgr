...
module AuthenticatesWithTwoFactor
	...
	def authenticate_with_two_factor_via_otp(user)
		if valid_otp_attempt?(user)
			...
			Rails.logger.info("Failed Login: user=#{user.cid} ip=#{request.remote_ip} method=OTP")
			prompt_for_two_factor(user)
		end
	end
	...
	def authenticate_with_two_factor_via_u2f(user)
		if U2fRegistration.authenticate(user, u2f_app_id, otp_params[:user][:device_response], session[:challenge])
			...
			Rails.logger.info("Failed Login: user=#{user.cid} ip=#{request.remote_ip} method=U2F")
	...
	def otp_params
		params.permit(:utf8, :authenticity_token, :commit, :otp_attempt, user: [:remember_me, :device_response])
	end
