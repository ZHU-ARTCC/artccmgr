class Profiles::TwoFactorAuths::U2fController < ApplicationController
	...
	def destroy
		@u2f_registration = U2fRegistration.find_by(user: current_user, id: params[:id])
		authorize @u2f_registration

		if @u2f_registration.destroy
			redirect_to profile_two_factor_auth_path, notice: 'Your U2F device was removed!'
		else
			redirect_to profile_two_factor_auth_path, alert: 'Unable to remove your U2F device!'
		end
	end
