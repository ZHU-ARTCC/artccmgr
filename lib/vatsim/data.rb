...
module VATSIM
  ...
  class Data
    ...
    def initialize(data_url)
      @url = data_url
      @raw = open(data_url.to_s, &:read)

      @raw = @raw.encode!(
        'US-ASCII', 'UTF-8', invalid: :replace, undef: :replace, replace: '?'
      )
      ...
      # TODO: rarely there are parsing issues with a singular user
      # Sanity check parsing versus client count
      # if @general.clients != (@atc.size + @pilots.size)
      # msg = 'Client count did not match number of parsed ATC and Pilots'
